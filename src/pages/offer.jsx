...
function Badge({ children, color = "gray", size = "sm" }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 border rounded-full px-2.5 py-0.5 text-xs font-medium ${colors[color]}`}
    >
      {children}
    </span>
  );
...
function OfertaCard({
  ...
  return (
    <article className="group relative bg-dark-800 border border-white/8 rounded-2xl overflow-hidden hover:border-brand/40 transition-all duration-300 hover:shadow-xl hover:shadow-brand/5 flex flex-col">
      {/* Franja superior de color por tipo */}
      <div
        className={`h-0.5 w-full ${
          oferta.tipo_oferta === "practicas"
            ? "bg-blue-500/60"
            : oferta.tipo_oferta === "practicas_contratacion"
              ? "bg-green-500/60"
              : "bg-purple-500/60"
        }`}
      />
      ...
      <div className="p-5 flex flex-col gap-3 flex-1">
        ...
        <div className="flex items-start gap-3">
          <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-brand/20 to-brand/5 border border-brand/15 flex items-center justify-center flex-shrink-0">
            {oferta.empresa_avatar ? (
              ...
              <Icon id="icon-building" className="w-5 h-5 text-brand" />
            )}
          </div>
          <div className="min-w-0 flex-1">
            <h3
              onClick={() => onVerDetalle(oferta)}
              className="font-display font-bold text-white text-[15px] leading-snug cursor-pointer hover:text-brand transition-colors line-clamp-2 pr-2"
            >
              {oferta.titulo}
            </h3>
            <p className="text-gray-500 text-xs mt-0.5 truncate">{empresa}</p>
          </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {oferta.modalidad && (
            <Badge color="gray">
              <Icon
                id={
                  oferta.modalidad === "Remoto" ? "icon-globe" : "icon-building"
                }
                className="w-3 h-3"
              />
              {oferta.modalidad}
          ...
          <div className="flex flex-wrap gap-1">
            {tecnologias.slice(0, 4).map((t) => (
              <span
                ...
                className="bg-white/5 border border-white/8 text-gray-400 text-[11px] px-2 py-0.5 rounded-md font-mono"
              >
                {t.nombre ?? t}
              </span>
            ))}
            {tecnologias.length > 4 && (
              <span className="text-gray-600 text-[11px] px-1.5 py-0.5">
                +{tecnologias.length - 4}
              </span>
            )}
          </div>
        ...
        {/* Metadata grid */}
        <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mt-auto pt-3 border-t border-white/5">
          {oferta.ubicacion && (
          ...
          {oferta.duracion_semanas && (
            <div className="flex items-center gap-1.5 text-gray-500 text-xs">
              <Icon id="icon-clock" className="w-3 h-3 flex-shrink-0" />
              <span>{oferta.duracion_semanas} semanas</span>
            </div>
          )}
          {oferta.horas_semanales && (
            <div className="flex items-center gap-1.5 text-gray-500 text-xs">
              <Icon id="icon-hourglass" className="w-3 h-3 flex-shrink-0" />
          ...
          {oferta.num_plazas_restantes != null && (
            <div className="flex items-center gap-1.5 text-gray-500 text-xs">
              <Icon id="icon-user" className="w-3 h-3 flex-shrink-0" />
              <span>
      ...
      <div className="px-5 pb-4 flex gap-2">
        ...
        <button
          onClick={() => onVerDetalle(oferta)}
          className="flex-1 text-xs py-2 rounded-xl bg-white/5 hover:bg-white/8 text-gray-400 hover:text-white transition-all border border-white/8 hover:border-white/15 font-medium"
        >
          Ver detalle
        </button>
        ...
        {isEstudiante &&
          (yaPostulado ? (
            <button
              onClick={() => onCancelarPostulacion(oferta)}
              className="flex-1 text-xs py-2 rounded-xl bg-red-500/10 hover:bg-red-500/15 text-red-400 hover:text-red-300 transition-all border border-red-500/20 font-medium flex items-center justify-center gap-1.5"
            >
              <svg
                className="w-3 h-3"
                viewBox="0 0 24 24"
            ...
            <button
              ...
              className="flex-1 text-xs py-2 rounded-xl bg-brand/15 hover:bg-brand/25 text-brand transition-all border border-brand/25 font-semibold flex items-center justify-center gap-1.5"
            >
              <svg
                className="w-3 h-3"
                viewBox="0 0 24 24"
        ...
        {isEmpresa && (
          <>
            <button
              ...
              className="flex-1 text-xs py-2 rounded-xl bg-white/5 hover:bg-white/8 text-gray-400 hover:text-white transition-all border border-white/8 font-medium"
            >
              Editar
            </button>
            <button
              ...
              className="flex-1 text-xs py-2 rounded-xl bg-red-500/5 hover:bg-red-500/10 text-red-500/70 hover:text-red-400 transition-all border border-red-500/15 font-medium"
            >
              Eliminar
            </button>
          </>
        ...
        <div className="p-6 space-y-5">
          ...
          {!esEdicion && (
            <div className="bg-brand/5 border border-brand/20 rounded-xl px-4 py-3 flex gap-3">
              ...
              <p className="text-xs text-gray-400">
                ...
                visibles. El proceso suele tardar menos de 24 h.
              </p>
            </div>
          )}
          <div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              placeholder="Nivel de inglés, carnet de conducir..."
              className="input-field resize-none"
            />
          </div>
          <div>
            ...
            <textarea
              ...
              rows={2}
              placeholder="Teletrabajo flexible, formación interna..."
              className="input-field resize-none"
            />
          </div>
    ...
    >
      <div className="bg-dark-800 border border-white/10 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative">
        ...
        <div className="p-6 space-y-5">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {tecnologias.map((t) => (
                  <span
                    key={t.id_tecnologia}
                    className="bg-brand/10 border border-brand/20 text-brand text-xs px-2.5 py-1 rounded-full font-mono"
                  >
                    {t.nombre}
                  </span>
                ))}
          ...
          {/* Acción estudiante en detalle */}
          {isEstudiante && (
            <div className="pt-2 border-t border-white/10">
              {yaPostulado ? (
                <div className="flex flex-col gap-3">
                  <div className="flex items-center justify-center gap-2 py-3 text-brand text-sm font-medium bg-brand/5 rounded-xl border border-brand/15">
                    <svg
                      className="w-4 h-4"
                      viewBox="0 0 24 24"
                      fill="none"
                  ...
                  <button
                    onClick={() => {
                      onCancelarPostulacion(oferta);
                      onClose();
                    }}
                    className="w-full py-2.5 rounded-xl bg-red-500/10 hover:bg-red-500/15 text-red-400 hover:text-red-300 transition-all border border-red-500/20 text-sm font-medium flex items-center justify-center gap-2"
                  >
                    <svg
                      className="w-4 h-4"
                      viewBox="0 0 24 24"
...
// ─── Modal postulación ──────────────────────────────────────────────────────
function PostulacionModal({ oferta, onClose, onSuccess, candidaturasActivas }) {
  const { user } = useAuth();
  const [mensaje, setMensaje] = useState("");
  const [sending, setSending] = useState(false);
  const [error, setError] = useState(null);

  // Límite de 5 candidaturas activas
  const limiteAlcanzado = candidaturasActivas >= 5;
  ...
  return (
    ...
    >
      <div className="bg-dark-800 border border-white/10 rounded-2xl w-full max-w-md p-6">
        ...
        {/* Aviso límite */}
        {limiteAlcanzado && (
          <div className="mb-4 bg-orange-500/10 border border-orange-500/30 rounded-xl px-4 py-3 flex gap-3">
            <Icon
              id="icon-warning"
              className="w-4 h-4 text-orange-400 flex-shrink-0 mt-0.5"
            />
            <p className="text-orange-300 text-sm">
              Has alcanzado el límite de 5 candidaturas activas. Retira alguna
              antes de postularte a nuevas ofertas.
            </p>
          </div>
        )}
        ...
        {!limiteAlcanzado && (
          <div className="mb-4 bg-white/3 border border-white/8 rounded-xl px-4 py-2.5 flex items-center justify-between">
            <span className="text-gray-500 text-xs">Candidaturas activas</span>
            <span
              className={`text-xs font-semibold ${candidaturasActivas >= 4 ? "text-orange-400" : "text-gray-300"}`}
            >
              {candidaturasActivas} / 5
            </span>
          </div>
        )}
...
// ─── Modal confirmar cancelación candidatura ─────────────────────────────────
function CancelarCandidaturaModal({ oferta, onClose, onConfirm }) {
  ...
  return (
    <div
      ...
      onClick={(e) => e.target === e.currentTarget && onClose()}
    >
      <div className="bg-dark-800 border border-white/10 rounded-2xl w-full max-w-sm p-6 text-center">
        ...
        <p className="text-gray-400 text-sm mb-6">
          ¿Confirmas que quieres retirar tu candidatura a{" "}
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 py-2.5 rounded-xl bg-red-500/15 hover:bg-red-500/25 text-red-400 border border-red-500/25 text-sm font-medium transition-all disabled:opacity-60 flex items-center justify-center gap-2"
          >
            {loading ? <Spinner className="w-4 h-4" /> : "Retirar"}
          </button>
        </div>
      </div>
    </div>
...
// ─── Panel "Mis candidaturas" (solo estudiante) ─────────────────────────────
    ...
    return (
      <div className="text-center py-16 border border-dashed border-white/10 rounded-2xl">
        ...
        <p className="text-gray-500 text-sm font-medium">
          Todavía no te has postulado a ninguna oferta
        </p>
        <p className="text-gray-600 text-xs mt-1">
          Explora las ofertas disponibles y envía tu primera candidatura
        </p>
      </div>
    );
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="text-gray-500 text-sm">
          ...
          {candidaturas.length !== 1 ? "s" : ""} activa
        ...
        <div className="flex items-center gap-2">
          <div className="h-1.5 w-28 rounded-full bg-white/10 overflow-hidden">
            <div
              ...
              style={{ width: `${(candidaturas.length / 5) * 100}%` }}
            />
          </div>
          <span
            className={`text-xs font-semibold ${candidaturas.length >= 4 ? "text-orange-400" : "text-gray-400"}`}
          >
      ...
      {candidaturas.map((c) => {
        const estado =
          ESTADO_CANDIDATURA[c.estado] ?? ESTADO_CANDIDATURA.pendiente;
        return (
          <div
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <button
                onClick={() => onVerOferta(c)}
                className="font-display font-semibold text-white text-sm hover:text-brand transition-colors truncate block w-full text-left"
              >
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              {(c.estado === "pendiente" || c.estado === "revision") && (
                <button
...
export default function OfertasPage() {
  ...
  const [activeTab, setActiveTab] = useState("ofertas"); // 'ofertas' | 'candidaturas'

  const [ofertas, setOfertas] = useState([]);
  const [loading, setLoading] = useState(true);

  // Candidaturas del estudiante
  const [candidaturas, setCandidaturas] = useState([]);
  const [loadingCandidaturas, setLoadingCandidaturas] = useState(false);
  // Set de id_oferta donde ya está postulado
  ...
  const [cancelarCandidatura, setCancelarCandidatura] = useState(null); // { id_candidatura, oferta }
  ...
  // ── Cargar candidaturas del estudiante ─────────────────────────────────
  const cargarCandidaturas = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select(
        `
        id_candidatura, estado, fecha_solicitud, mensaje, id_oferta,
        oferta:id_oferta(titulo, empresa:empresa(nombre, id_usuario, usuario:id_usuario(avatar_url)))
      `,
      )
      .eq("id_estudiante", user.id)
      .order("fecha_solicitud", { ascending: false });

    const candidaturasNorm = (data ?? []).map((c) => ({
      ...c,
      oferta_titulo: c.oferta?.titulo ?? "Oferta",
      empresa_nombre: c.oferta?.empresa?.nombre ?? "Empresa",
      empresa_avatar: c.oferta?.empresa?.usuario?.avatar_url ?? null,
    }));

    setCandidaturas(candidaturasNorm);
    setPostulaciones(new Set(candidaturasNorm.map((c) => c.id_oferta)));
  ...
  useEffect(() => {
    cargarOfertas();
    cargarCandidaturas();
  }, [cargarOfertas, cargarCandidaturas]);

  // ── Cancelar candidatura ───────────────────────────────────────────────
  const confirmarCancelacion = async () => {
    if (!cancelarCandidatura) return;
    await supabase
      .from("candidatura")
      .delete()
      .eq("id_candidatura", cancelarCandidatura.id_candidatura);
    await cargarCandidaturas();
    setCancelarCandidatura(null);
  };

  // Handler para retirar desde card / detalle
  const handleCancelarPostulacion = (oferta) => {
    const candidatura = candidaturas.find(
      (c) => c.id_oferta === oferta.id_oferta,
    );
    if (candidatura)
      setCancelarCandidatura({ ...candidatura, oferta_titulo: oferta.titulo });
  };
  ...
  // Ver oferta desde candidatura
  const handleVerOfertaDesdeCandidatura = (candidatura) => {
    const oferta = ofertas.find((o) => o.id_oferta === candidatura.id_oferta);
    if (oferta) {
      setDetalleOferta(oferta);
  ...
  const candidaturasActivas = candidaturas.filter(
    (c) => !["aceptada", "rechazada"].includes(c.estado),
  ).length;
    ...
    <MainLayout>
      <div className="min-h-screen bg-dark">
        <main className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          ...
          <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
            <div>
              <h1 className="font-display text-3xl font-bold text-white">
                {isEmpresa ? "Mis ofertas" : "Ofertas"}
              </h1>
              <p className="text-gray-500 text-sm mt-1">
                {isEmpresa
                  ? "Gestiona y publica tus ofertas de prácticas o empleo"
                  : "Encuentra prácticas y empleo junior"}
              </p>
            </div>
            {isEmpresa && (
              <button
          ...
          {isEstudiante && (
            <div className="flex items-center gap-1 bg-dark-800 border border-white/8 rounded-xl p-1 mb-6 w-fit">
              <button
                onClick={() => setActiveTab("ofertas")}
                className={`px-5 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === "ofertas" ? "bg-brand/15 text-brand border border-brand/20" : "text-gray-500 hover:text-gray-300"}`}
              >
                Ofertas
                <span className="ml-2 text-xs bg-white/8 px-1.5 py-0.5 rounded-full">
                  {ofertasFiltradas.length}
                </span>
              </button>
              <button
                onClick={() => setActiveTab("candidaturas")}
                className={`px-5 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${activeTab === "candidaturas" ? "bg-brand/15 text-brand border border-brand/20" : "text-gray-500 hover:text-gray-300"}`}
              >
                Mis candidaturas
                {candidaturasActivas > 0 && (
                  <span
                    className={`text-xs px-1.5 py-0.5 rounded-full ${candidaturasActivas >= 5 ? "bg-orange-500/20 text-orange-400" : "bg-white/8 text-gray-400"}`}
                  >
                    {candidaturasActivas}/5
                  </span>
                )}
          ...
          {/* ── Tab Candidaturas ── */}
          {isEstudiante && activeTab === "candidaturas" && (
            <MisCandidaturas
              candidaturas={candidaturas}
              loadingCandidaturas={loadingCandidaturas}
              onCancelar={(c) => setCancelarCandidatura(c)}
              onVerOferta={handleVerOfertaDesdeCandidatura}
            />
          )}
          ...
          {activeTab === "ofertas" && (
            <>
              ...
              <div className="bg-dark-800 border border-white/8 rounded-2xl p-4 mb-6 space-y-3">
                <div className="relative">
                  <svg
              ...
              {!isEmpresa && (
                <p className="text-gray-600 text-xs mb-4">
                  ...
                  {ofertasFiltradas.length !== 1 ? "s" : ""} encontrada
      ...
      {postulacionOferta && (
        <PostulacionModal
          ...
          candidaturasActivas={candidaturasActivas}
      ...
      {cancelarCandidatura && (
        <CancelarCandidaturaModal
          oferta={cancelarCandidatura}
          onClose={() => setCancelarCandidatura(null)}
