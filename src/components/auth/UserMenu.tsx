...
export default function UserMenu({ onClose }: { onClose: () => void }) {
  ...
  // ── Fix 1: el cleanup del canal no debe ser async ──
  useEffect(() => {
    if (!user) return;
  ...
  // ── Fix 2: eliminado "centro" que no existe en el tipo de userRole ──
  const profilePath =
    role === "admin"
      ? "/perfil/admin"
      : role === "empresa"
  ...
  return (
    ...
    >
      ...
      {/* Items — Fix 3: usar <a> en lugar de <span> para tener href */}
